...
class FirebaseManager: ObservableObject {
    ...
    func create(league: League, managerEmails: [String], user: User, posted: @escaping (_ posted: Bool) -> Void, failure: @escaping (_ failed: Error) -> Void) {
        ...
        let reference = db.reference().child("\(kLeagueEndpoint)\(league.name)").childByAutoId()
         
         guard let email = user.email else { return }
         
        let payload: [NSString : Any] = ["emails" : managerEmails, "creator_email" : email, "show" : league.program.title]
